...
(function($){
  ...
  //          해석을 하면 용어 그대로 읽으면 됨
  // 2. $.메소드() : 유틸리티메소드 - 어떠한 기능 처리
  //                해석을 하면 메소드 기준으로 읽으면 됨

  // $.ajax().done(function(){});

  // Asynchronous Javascript and XML(JSON) : 실시간(비동기)으로 자료 체크 기술 -> 비동기는 실시간 자료만 바뀌는 것, 동기 -> 새로고침 또는 아예 다른 페이지로 전환
  ...
  $.ajax({
    ...
    console.log('2. ajax 이용하여 파일 불러오기!');
    var importData = data;
  ...
  // setTimeout은 정해진 일정 시간 뒤에 기능을 수행하는 함수
  setTimeout(function(){
    console.log(dataFile);
    console.log('4. setTimeout 처리!');
  }, 500);  // 0.5초 안에 실행해라 // ajax 함수는 시간이 더 걸리기 때문에 얘를 먼저 실행하는 것
