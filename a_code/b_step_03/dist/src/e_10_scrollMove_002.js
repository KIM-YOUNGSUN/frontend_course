...
window.addEventListener('scroll',(e)=>{                // 브라우저에서 스크롤이 실행되었을 때
  const targetScroll = e.currentTarget.scrollY;        // 브라우저(this)가 스크롤이 움직인 값 파악
  let moveScroll = parseInt(targetScroll / 15);      // 스크롤 값을 줄이기 위해 일정값(15) 나눠 숫자
  // 이미지 중에서 moveScroll의 값에 해당하는 이미지는 나타나고 나머지는 사라지게 수행       
  ...
  elImgArr.forEach((img,idx)=>{                           
    // img.classList.remove(OPTION_ON);    // 이미지 각각 전부 .on 제거
    // elImgArr[moveScroll].classList.add(OPTION_ON); // moveScroll 순번의 이미지는 .on 첨부

    // moveScroll 값과 idx 순번을 비교하여 일치하면 나타나고 불일치하면 사라지게 수행
    (idx !== moveScroll) ? img.classList.remove(OPTION_ON) : img.classList.add(OPTION_ON);
  });
  // *이미지 담은 요소 이동수행 (topMove 음수로 나타나기 시작하는 시점)
  let topMove = COUNT_NUM + 20 - moveScroll  // 이미지가 모두 처리된 시점을 기준으로 해당 요소는 위로 이동하게 처리
  if(topMove <= 0){
